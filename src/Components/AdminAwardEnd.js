...
function AdminAwardEnd() {
  const postAwardURI = `${process.env.REACT_APP_SERVER_BASE_URL}/award`;
  const getAwardUri = `${process.env.REACT_APP_SERVER_BASE_URL}/getaward`;
  const editAwardUri = `${process.env.REACT_APP_SERVER_BASE_URL}/edit-award`;
  const deleteAwardUri = `${process.env.REACT_APP_SERVER_BASE_URL}/delete-award`;
  useEffect(() => {
    AOS.init();
    AOS.refresh();
  }, []);
  ...
  const editFunc = (award) => {
    axios.post(editAwardUri, award).then((res) => {
      if(res.data.status){
        window.location.reload()
      }
      else{
