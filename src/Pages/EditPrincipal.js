...
function EditPrincipal() {
  ...
  const PrincipalURI = `${process.env.REACT_APP_SERVER_BASE_URL}/from-principal`;
  useEffect(() => {
    AOS.init();
    AOS.refresh();
  }, []);
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_SERVER_BASE_URL}/to-principal`)
      .then((res) => {
        setprincipalInfo(() => {
        ...
        <div className="col-lg-10 mx-auto" key={index}>
          ...
          <div className="row">
            <div
              ...
              data-duration="1000"
            >
              <img src={ principal.principalImage!== '' ? principal.principalImage :image}  className="w-100 principal_image" alt="school principal"/>
